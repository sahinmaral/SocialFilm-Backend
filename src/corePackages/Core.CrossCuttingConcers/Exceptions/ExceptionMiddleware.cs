...
public class ExceptionMiddleware
{
    ...
    private Task CreateRegistrationFailedException(HttpContext context, Exception exception)
    {
        context.Response.StatusCode = Convert.ToInt32(HttpStatusCode.BadRequest);
        object errors = ((RegistrationFailedException)exception).Errors;

        return context.Response.WriteAsync(new RegistrationFailedProblemDetails
        {
            Status = StatusCodes.Status400BadRequest,
            Type = "https://example.com/probs/registrationFailed",
            Title = "Registration failed exception",
            Detail = "",
            Instance = "",
            Errors = errors
        }.ToString());
